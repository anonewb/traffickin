...
self.addEventListener('install', function(event) {
	//***this install stage will be triggered when user visit's pg for first time
	console.log('[Service Worker] Installing Service Worker ...', event);
	event.waitUntil(
		caches
			.open(CACHE_STATIC_NAME) //CACHE_STATIC_NAME will be the name of the cache. GOTO Application>cache storage
			.then(function(cache) {
...
self.addEventListener('activate', function(event) {
	...
	event.waitUntil(
		// activate event is waited until this fn is finished executing
		caches
			.keys() //keys returns with names of array of [cached files both static-v2 and dynamic-v1]
			.then(function(keyList) {
				return Promise.all(
					keyList.map(function(key) {
						// Promise.all takes all promises and waits untill they are finished
...
self.addEventListener('fetch', function(event) {
	...
	if (event.request.url.indexOf(url) > -1) {
		...
		//STRATEGY: CACHE ONLY STRATEGY
		event.respondWith(caches.match(event.request));
	} else {
		//STRATEGY: CACHE FIRST, THEN NETWORK
		event.respondWith(
			caches
				.match(event.request) //caches refers to overall cache storage
				.then(function(response) {
					...
					if (response) {
						...
						return fetch(event.request) //reach out to network if we dont have it in cache
							.then(function(res) {
								return caches
									.open(CACHE_DYNAMIC_NAME) //then we store the res in 'CACHE_DYNAMIC_NAME'
									.then(function(cache) {
							...
							.catch(function(err) {
								//if network also fails, i.e: providing offline fallback pg
								return caches
									.open(CACHE_STATIC_NAME) //open means storing in cache
									.then(function(cache) {
										if (
				...
				.then(function(data) {
					for (var dt of data) {
						...
						fetch(
							...
							{
								// posting/sending data that we want to store in server
								method: 'POST',
								body: postData
							}
						)
